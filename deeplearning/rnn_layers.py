...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(1, T + 1):
        h[:, i, :], cache = rnn_step_forward(x[:, i - 1, :], h[:, i - 1, :], Wx, Wh, b)
        caches.append(cache)
    h, cache = h[:, 1:, :], caches
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    prev_h, next_h, Wx, Wh, b, x = cache[-1]
    N, T, H = dh.shape
    N, D = x.shape
    ...
    for i in range(T - 1, -1, -1):
        ...
        dx[:, i, :], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache[i])
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # Getting words from the first dim of W (numpy takes care of reshaping!)
    out = W[x, :]
...
def word_embedding_backward(dout, cache):
    ...
    # Adding upstream gradient to the corresponding indices of dW
    np.add.at(dW, x, dout)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # Partial Derivative of o
    do = dnext_h * np.tanh(next_c)
    # Derivative of next_h w.r.t. tanh(next_c)
    ...
    # Stacking back da_i, da_f, da_o, da_g to get da
    da = np.hstack((da_i, da_f, da_o, da_g))
    # Using da to find dx, dprev_h, dWx, dWh, db
    dx, dprev_h, dWx, dWh, db = da @ Wx.T, da @ Wh.T, x.T @ da, prev_h.T @ da, np.sum(da, axis=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    caches = []
    for i in range(1, T + 1):
        h[:, i, :], prev_c, cache = lstm_step_forward(x[:, i - 1, :], h[:, i - 1, :], prev_c, Wx, Wh, b)
        caches.append(cache)
    h, cache = h[:, 1:, :], caches
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dnext_h = dprev_h + dh[:, i, :]
        dx[:, i, :], dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h, dprev_c, cache[i])
        dWx, dWh, db = dWx + dWx_t, dWh + dWh_t, db + db_t
    dh0 = dprev_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
